...
export default function GameCard({ game }) {
  ...
  return (
    <div className={`game-card rounded-2xl p-6 flex flex-col justify-between relative overflow-hidden transition-all duration-200 hover:-translate-y-1 ${
      game.isFeatured ? 'border-2 border-sky-300 bg-gradient-to-b from-sky-50/70 to-white shadow-xl shadow-sky-500/10' : 'bg-white border border-slate-200'
    }`}>
      {/* Featured Ribbon Badge */}
      {game.isFeatured && (
        <div className="absolute top-3 right-3 flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-sky-100 border border-sky-300 text-sky-800 text-[10px] font-extrabold uppercase tracking-wider">
          <Star className="w-3 h-3 text-sky-600 fill-sky-600" />
          <span>Featured</span>
        </div>
      )}
      ...
      <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
        ...
        <Link
          to={game.path}
          className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-bold text-sm transition-all duration-150 shadow-md shadow-sky-500/25 hover:shadow-sky-500/40 cursor-pointer"
        >
          <span>Play</span>
